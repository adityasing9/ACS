...
def connect():
    ...
    join_room("public")
    emit("identity", name)
    emit("system", f"{name} joined the chat", room="public")

@socketio.on("message")
def message(msg):
    emit("message", msg, room="public")
...
def change_name(new_name):
    ...
    old_name = users.get(request.sid)
    users[request.sid] = new_name

    emit("identity", new_name)
    emit("system", f"{old_name} changed name to {new_name}", room="public")

@socketio.on("disconnect")
def disconnect():
    name = users.pop(request.sid, None)
    if name:
        emit("system", f"{name} left the chat", room="public")
